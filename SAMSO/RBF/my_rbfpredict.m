function [Yq] = my_rbfpredict(model, Xtr, Xq)
% RBFPREDICT
...
for t = 1 : nq
    switch upper(model.bf_type)
        ...
        case 'CUB'
            ...
            %    dist(j) = 1 / sqrt(sum((Xq(t,:) - Xtr(j,:)).^2) + model.bf_c^2);
            %end
            dist = sqrt(sum((repmat(Xq(t,:),model.n,1) - Xtr).^2,2) + model.bf_c^2).^3;
    ...
    if model.poly == 0
    Yq(t) = model.meanY + model.coefs' * dist;%Yq(t) =   model.coefs' * dist;     
    elseif model.poly ==1
        Yq(t) = model.coefs' * [dist; 1; Xq(t,:)'];
    elseif model.poly ==2
        XQ=dace_regpoly2(Xq(t,:));
        Yq(t) = model.coefs' * [dist; XQ'];
    end
end
