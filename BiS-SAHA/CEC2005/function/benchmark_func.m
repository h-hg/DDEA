...
if initial_flag==0
    ...
    %从此行开始是测试5个原始的，没有经过变换的函数
    elseif func_num==101 fhd=str2func('fellipsoid'); %[-5.12,5.12] 
    elseif func_num==102 fhd=str2func('frosenbrock'); %[-2.048,2.048] 
    elseif func_num==103 fhd=str2func('fackley'); %[-32.768,32.768] 
    elseif func_num==104 fhd=str2func('fgriewank'); %[-600,600] 
    elseif func_num==105 fhd=str2func('frastrigin'); %[-5,5] 
    end
    load fbias_data;
end
if func_num <100    %rzhh 为了测试自己增加的函数
...
for i = 1:ps
    for j = 1:(D-1)
        sum(i, 1) = sum(i, 1) + 100*(x(i, j+1) - x(i, j)^2)^2 + (x(i, j) - 1)^2;
    end
end
f = sum;
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [y] = fellipsoid(x)
% % INPUT:
% % xx = [x1, x2, ..., xd]
[ps,D] = size(x);
sum = zeros(ps, 1);
