...
%                      label_FUN = @(x)RBF_eval(x,temp_trainset1,labellambda,labelgamma,flag);
                     ...
                     for j=1:popsize
                         if current_approximated(j,1) == 0
                             %sum_error(i,1) = sum_error(i,1) + approx_diff(j,1)-abs(FUN(current_position(j,:))-current_fitness(j,1));%+approx_diff(j,1) - abs(label_FUN(current_position(j,:))-current_fitness(j,1));
                             sum_error(i,1) = sum_error(i,1) + approx_diff(j,1) - abs(label_FUN(current_position(j,:))-current_fitness(j,1));
                         end
                     end
                 end
             end
             [maxvalue maxid] = max(sum_error);
