...
for r=1:run_times
    ...
    for i=1:popsize
        ...
        label_position(evaltimes+1,:) = current_position(i,:);
        label_fitness(evaltimes+1,1) = current_fitness(i,:);
    ...
    gbest_output(1)=gbest_fitness(1,1);
     ...
     while evaltimes<max_evaluations
         ...
         if evaltimes<3*dimension
%              if train_num>dimension*2
%                  randtrainset = randperm(train_num);
%                  trainset1 = trainset;
%                  trainset_fitness1 = trainset_fitness;
%                  trainset = trainset1(randtrainset(1:2*dimension),:);
%                  trainset_fitness = trainset_fitness1(randtrainset(1:2*dimension),1);
%              end
         else
             trainset = label_position;
             trainset_fitness = label_fitness;
         end
...
%          spread = sqrt(spread)*(mean(trainset_fitness))/dimension; 
         ...
         for i=1:popsize
             if i~=min_neighborid & i~=maxdiff_id
                 if min_neighborid~=0 & (approx_diff(i,1) - abs(approximated_fitness(min_neighborid,1) - current_fitness(min_neighborid,1)))<-1e-3
                     archive_position(n_archive,:) = current_position(i,:);
                     archive_fitness(n_archive,1) = current_fitness(i,1);
                     n_archive = n_archive+1;
                     temp_archive = [temp_archive;i];
                 end
                 if min_neighborid==0 & (approx_diff(i,1) - abs(approximated_fitness(maxdiff_id,1) - current_fitness(maxdiff_id,1)))<-1e-3
                     archive_position(n_archive,:) = current_position(i,:);
                     archive_fitness(n_archive,1) = current_fitness(i,1);
                     n_archive = n_archive+1;
                     temp_archive = [temp_archive;i];
