...
%                  current_velocity(i,d) = 0.7293*(current_velocity(i,d) + 2.05*rand()*(pbest(i,d) - current_position(i,d)) + 2.05*rand()*(gbest(1,d) - current_position(i,d)));
         ...
         setsize = max_evaluations;%5*(dimension);%
         train_num = 1;
         trainset = [];
         trainset_fitness = [];
         [sortvalue sortid] = sort(archive_fitness,'ascend');%sort(archive_iter,'descend');%
        ...
        FUN = @(x)RBF_eval(x,trainset,lambda,gamma,flag);
         ...
         for i=1:popsize
             if approximated_fitness(i,1)<pbest_fitness(i,1) & labelapproximated_fitness(i,1)<pbest_fitness
                 ...
                 approx_diff(i,1) = abs(current_fitness(i,1) - approximated_fitness(i,1));
         ...
         if hasevaluated == 1
             evalid = find(current_approximated == 0);
             temp_trainset = labeltrainset;
             temp_trainsetfit = labeltrainset_fitness;
             temp_trainset=[temp_trainset;current_position(evalid,:)];
             temp_trainsetfit = [temp_trainsetfit; current_fitness(evalid,1)];
             flag ='cubic';
             ...
             for i=1:popsize
                 if current_approximated(i,1) == 1
                     error_diff(i) = abs(current_fitness(i,1) - FUN(current_position(i,:)));
                 end
             end
             [minvalue1 minid1] = min(error_diff);
             archive_position(n_archive,:) = current_position(minid1,:);
             archive_fitness(n_archive,1) = current_fitness(minid1,1);
             archive_iter(n_archive,1) = iter;
             n_archive = n_archive + 1;
         end
