...
%--------------------------------------------------------------------------
    ...
    [FrontNo,MaxFNo] = NDSort(Population.obj,[],N);
    Next = FrontNo < MaxFNo;
    
    %% Select the solutions in the last front
    Last   = find(FrontNo==MaxFNo);
    Choose = LastSelection(Population.obj(Next,:),Population.obj(Last,:),N-sum(Next),Z,Zmin);
    Next(Last(Choose)) = true;
    % Population for next generation
    Population.dec = Population.dec(Next,:);
    Population.obj = Population.obj(Next,:);
end
