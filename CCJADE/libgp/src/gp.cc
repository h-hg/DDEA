...
namespace libgp {
  ...
  const double initial_L_size = 10;
  ...
  tFitness GaussianProcess::f(const double x[])
  {
  ...
  std::vector<double> GaussianProcess::df(const double x[])
  {
	  std::vector<double> gradient(input_dim, 0.0);	  

	  if (sampleset->empty()) return gradient;	  
	  ...
	  Eigen::VectorXd c;
	  c.resize(sampleset->size());
	  for (int i = 0; i < sampleset->size(); ++i)
		  c(i) = alpha(i)*k_star(i);

	  Eigen::MatrixXd XT(input_dim, sampleset->size());
	  for (int i = 0; i < input_dim; ++i)
		  for (int j = 0; j < sampleset->size(); ++j)
			  XT(i, j) = x[i] - sampleset->x(j)(i);
	  
	  for (int i = 0; i < input_dim; ++i)
	  {
		  gradient[i] = 0;
		  for (int j = 0; j < sampleset->size(); ++j)
			  gradient[i] += XT(i, j)*c(j);
	  }
	  
	  Eigen::VectorXd l = cf->get_characteristic_length();
	  for (int i = 0; i < input_dim; ++i)
		  gradient[i] = -gradient[i] / l(i);
	  return gradient;
  }
  ...
  tFitness GaussianProcess::var(const double x[])
  {
    if (sampleset->empty()) return 0;
    Eigen::Map<const Eigen::VectorXd> x_star(x, input_dim);
    compute();
  ...
  void GaussianProcess::add_pattern(const double x[], tFitness y)
  {
  ...
  bool GaussianProcess::set_y(size_t i, tFitness y)
  {
