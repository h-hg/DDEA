...
namespace libgp {
  ...
  public:
    ...
    virtual tFitness f(const double x[]);

	/** Predict derivative of target value for given input.
	*  @param x input vector
	*  @return predicted derivative value */
	virtual std::vector<double> df(const double x[]);
    
    /** Predict variance of prediction for given input.
     *  @param x input vector
     *  @return predicted variance */
	virtual tFitness var(const double x[]);
     ...
     */
	void add_pattern(const double x[], tFitness y);


	bool set_y(size_t i, tFitness y);
